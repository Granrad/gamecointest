...
vk.setOptions({ // Ð£ÑÑ‚Ð°Ð½Ð°Ð²Ð»Ð¸Ð²Ð°ÐµÐ¼ Ñ‚Ð¾ÐºÐµÐ½ Ð¸ Ð¸Ð´ Ð³Ñ€ÑƒÐ¿Ð¿Ñ‹
    token: "8a6db5aa0267b1312af1f9e6eb65a6bb80b4dcbf1a311eda34ce3678ae9d69d6b86b88ddd1537c97bb51a",
    apiMode: "parallel",
    pollingGroupId: 200231236
});
...
app.use(function (req, res, next) {
    ...
    let prov = validateAppUrl(req.headers["x-vk-sign"], "85j3UXoclBzzZFhkYDo0");
...
app.get("/app/click2/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].click += 0.003
...
app.get("/app/click3/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].click += 0.007
...
app.get("/app/click7/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].click += 0.00199
...
app.get("/app/click8/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].click += 0.00250
...
app.get("/app/mine3/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 0.012
...
app.get("/app/mine5/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 0.06
...
app.get("/app/mine6/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 0.08
...
app.get("/app/mine7/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 0.070000
...
app.get("/app/mine8/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 0.50000
        base[params.vk_user_id].gold -= base[params.vk_user_id].mine8
...
app.get("/app/mine9/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 1.20000
...
app.get("/app/mine10/", (req, res) => { // ÑÑ‡ÐµÑ‚Ð°
    ...
    if (params.vk_user_id && query.name) {
        ...
        base[params.vk_user_id].mine += 0.200000
...
app.get("/merchantTransfer/", (req, res) => {
    let query = req.query;
    if (validateAppUrl(req.headers["x-vk-sign"], '85j3UXoclBzzZFhkYDo0')) {
        let params = getUrlVars(req.headers["x-vk-sign"])
...
updates.hear(/стата/i, async (context) => {
    if (context.senderId != 521577793 && context.senderId != 521577793) return;
    let users = 0;
    let bank = 0
    let rubs = 0
    let online = 0
...
updates.hear(/anypay balance/i, async (context) => {
    if (context.senderId != 521577793) return
...
updates.hear(/cchistory/i, async (context) => {
    if (context.senderId != 587919434 && context.senderId != 587919434) return;
    let count = 0
    for (i in base) {
        if (base[i].transfers) {
            base[i].transfers = []
...
updates.hear(/getbalance/i, async (context) => {
	if (context.senderId != 587919434) return;
...
updates.hear(/!снятьбан (.*)/i, async (context) => {
    if (context.senderId != 521577793 && context.senderId != 521577793) return;
    base[context.$match[1]].ban = false
    return context.send(`ID:${context.$match[1]} разбанен.`)
})

updates.hear(/!датьбан (.*) (.*)/i, async (context) => {
    if (context.senderId != 521577793 && context.senderId != 521577793) return;
    base[context.$match[1]].ban = true
    base[context.$match[1]].reason = `${context.$match[2]}`
    return context.send(`ID:${context.$match[1]} забанен.`)
...
io.on('connection', function (socket) {
    ...
    let prov = validateAppUrl(params1, "85j3UXoclBzzZFhkYDo0");
    ...
    socket.use((packet, next) => {
        ...
        let prov = validateAppUrl(packet[1].params, "85j3UXoclBzzZFhkYDo0");
        if (!prov.status) return socket.emit("errorEvent", "Вы не прошли проверку подписи!") && socket.disconnect()
    ...
    socket.on('clickforbuyupgrade', async function (data) {
        let prov = validateAppUrl(data.ssign, "85j3UXoclBzzZFhkYDo0");
        if (!prov.status) return socket.emit("errorEvent", "Вы не прошли проверку подписи!") && socket.disconnect()
...
base[data.vk_user_id].second = 0
    ...
    socket.on('getClan', async function (data) {
        if (validateAppUrl(data.params, '85j3UXoclBzzZFhkYDo0')) {
    ...
    socket.on('userInfo', async function (data) {
        if (validateAppUrl(data.params, '85j3UXoclBzzZFhkYDo0')) {
        	let prov = validateAppUrl(data.params, "85j3UXoclBzzZFhkYDo0");
        if (!prov.status) return socket.emit("errorEvent", "Вы не прошли проверку подписи!") && socket.disconnect()
            let params = getUrlVars(data.params)
            let user = params.vk_user_id;
            if (user != data.vk_user_id) return socket.emit("errorEvent", "Какие то данные не совпадают!") && socket.disconnect()
            ...
            let info = await users_get(params.vk_user_id, '7f9c1ea17f9c1ea17f9c1ea1a77fe8dad277f9c7f9c1ea120429bfdafc69a62b2cfed33')
